      ...
      <div className="flex-1 overflow-y-auto px-3 md:px-4 py-4 md:py-6">
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {phrases.map((phrase, index) => (
            ...
            >
              <span className="text-sm md:text-base text-gray-800 dark:text-gray-200 text-left flex-1">
                {phrase}
              </span>
              {copiedIndex === index ? (
                <Check className="w-4 h-4 text-green-500 flex-shrink-0" />
              ) : (
                <Copy className="w-4 h-4 text-gray-400 group-hover:text-sky-600 dark:group-hover:text-sky-400 flex-shrink-0 transition-colors" />
              )}
            </button>
          ))}
        </div>
